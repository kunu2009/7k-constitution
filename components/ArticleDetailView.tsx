...
const DetailSection: React.FC<{ title: string; borderColorClass: string; children: React.ReactNode }> = ({ title, borderColorClass, children }) => (
    <div className={`bg-light-background/80 dark:bg-slate-800/60 p-4 rounded-lg border-l-4 ${borderColorClass} mb-6 shadow-sm`}>
        <h2 className="text-xs font-semibold uppercase text-light-text-secondary dark:text-gray-400 tracking-wider mb-2">{title}</h2>
        {children}
    </div>
);
...
const LandmarkCaseCard: React.FC<{ caseItem: LandmarkCase }> = ({ caseItem }) => (
  <div className="mb-4 last:mb-0">
    <h3 className="font-semibold text-md text-light-text dark:text-gray-200">{caseItem.caseName}</h3>
    <p className="text-sm text-light-text-secondary dark:text-gray-400 mt-1">{caseItem.caseSummary}</p>
  </div>
);
...
const ArticleDetailView: React.FC<ArticleDetailViewProps> = ({ article, userData, onClose, onToggleFavorite, onUpdateNotes }) => {
  ...
  return (
    <div className="flex flex-col h-full bg-light-background dark:bg-gray-900">
      <header className="flex-shrink-0 bg-light-surface dark:bg-gray-800 shadow-md p-4 flex items-center justify-between z-10">
        <button onClick={onClose} className="px-4 py-2 rounded-md bg-black/5 dark:bg-gray-700 hover:bg-black/10 dark:hover:bg-gray-600 font-semibold transition-colors">
          &larr; Back
        </button>
        ...
        <button 
          ...
          className={`p-2 rounded-full transition-colors ${isFavorite ? 'text-yellow-500 bg-yellow-100 dark:bg-yellow-900' : 'text-gray-400 hover:bg-black/5 dark:hover:bg-gray-700'}`}
      ...
      <div className="flex-grow p-4 sm:p-8 overflow-y-auto">
        <div className="max-w-4xl mx-auto bg-light-surface dark:bg-gray-800 p-6 rounded-xl shadow-lg">
          ...
          <DetailSection title="Summary" borderColorClass="border-blue-500">
            <p className="text-lg text-light-text dark:text-gray-200 leading-relaxed">{article.summary}</p>
          </DetailSection>

           <DetailSection title="Detailed Explanation" borderColorClass="border-navy">
            <p className="text-base text-light-text dark:text-gray-300 leading-relaxed">{article.detailedExplanation}</p>
          </DetailSection>

          <DetailSection title="Limitations and Exceptions" borderColorClass="border-yellow-500">
            <p className="text-base text-light-text dark:text-gray-300 leading-relaxed">{article.limitationsAndExceptions}</p>
          </DetailSection>
          
          <DetailSection title="Full Text" borderColorClass="border-green">
            <p className="text-base text-light-text dark:text-gray-300 leading-relaxed whitespace-pre-wrap font-serif">{article.fullText}</p>
          </DetailSection>
          ...
          <hr className="my-8 border-black/10 dark:border-gray-700" />
          ...
          <div>
            ...
            <textarea
              ...
              className="w-full h-40 p-3 bg-light-background/80 dark:bg-gray-700 rounded-lg border border-black/10 dark:border-gray-600 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition"
            />
          </div>
